...
async function copyChatFilePathIdx(idx) {
    ...
    if (ok) {
        const msg = (typeof window.t === 'function') ? window.t('chatFilesPage.pathCopied') : 'Path copied. Paste into a conversation to reference.';
...
function chatFilesEditBlockedHint() {
    return (typeof window.t === 'function')
        ...
        : 'Binary files (images, archives, etc.) cannot be edited as text here. Please use "Download".';
}
...
function renderChatFilesTable() {
    ...
    const tCopyTitle = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.copyPathTitle') : 'Copy absolute server path for pasting into conversation');
    const tDlTitle = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.download') : 'Download');
    const tMoreTitle = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.moreActions') : 'More');
    ...
    function rowHtml(f, idx) {
        ...
        const editUnavailable = (typeof window.t === 'function') ? escapeHtml(window.t('chatFilesPage.editUnavailable')) : 'Cannot edit';
...
async function deleteChatFolderFromBrowse(folderName) {
    ...
    const rel = segs.join('/');
    const q = (typeof window.t === 'function') ? window.t('chatFilesPage.confirmDeleteFolder') : 'Delete this folder and all files inside?';
    if (!confirm(q)) return;
    try {
            ...
            if (res.status === 404) {
                ...
                if (/not\s*found/i.test(String(errMsg))) {
                    ...
                    const cleared = (typeof window.t === 'function')
                        ...
                        : 'Directory does not exist on server. List refreshed.';
    ...
    try {
        ...
        if (data && data.ok) {
            const msg = (typeof window.t === 'function')
                ...
                : 'Upload complete. Click "Copy Path" in the list to paste into a conversation.';
            chatFilesShowToast(msg);
        }
    } catch (e) {
        alert((e && e.message) ? e.message : String(e));
