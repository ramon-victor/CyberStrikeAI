...
func (h *MarkdownAgentsHandler) CreateMarkdownAgent(c *gin.Context) {
	...
	if want := agents.WantsMarkdownOrchestrator(filepath.Base(path), body.Kind, string(out)); want {
		...
		if other != "" {
			c.JSON(http.StatusConflict, gin.H{"error": fmt.Sprintf("Primary agent definition already exists: %s, please delete or remove its primary agent marker first", other)})
			return
		}
	}
	...
	if h.audit != nil {
		h.audit.RecordOK(c, "agent", "markdown_create", "Created Markdown sub-agent", "markdown_agent", filepath.Base(path), nil)
	}
...
func (h *MarkdownAgentsHandler) UpdateMarkdownAgent(c *gin.Context) {
	...
	if h.audit != nil {
		h.audit.RecordOK(c, "agent", "markdown_update", "Updated Markdown sub-agent", "markdown_agent", filename, nil)
	}
...
func (h *MarkdownAgentsHandler) DeleteMarkdownAgent(c *gin.Context) {
	...
	if h.audit != nil {
		h.audit.RecordOK(c, "agent", "markdown_delete", "Deleted Markdown sub-agent", "markdown_agent", filename, nil)
	}
